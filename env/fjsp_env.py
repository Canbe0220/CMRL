...
class EnvState:
    ...
    feat_image_batch: torch.Tensor = None
    curr_proc_batch: torch.Tensor = None
    proc_times_batch: torch.Tensor = None
    re_proc_times_batch: torch.Tensor = None
    ...
    def update(self, batch_idxes, feat_opes_batch, feat_mas_batch, feat_image_batch, curr_proc_batch, proc_times_batch, re_proc_times_batch, ope_ma_adj_batch,
        ...
        self.feat_image_batch = feat_image_batch
        self.curr_proc_batch = curr_proc_batch
        self.proc_times_batch = proc_times_batch
        self.re_proc_times_batch = re_proc_times_batch
...
class FJSPEnv(gym.Env):
    ...
    def __init__(self, case, env_paras, data_source='case'):
        ...
        max_opes = torch.max(self.nums_ope_batch).item()
        self.max_proc = torch.max(self.proc_times_batch).item()
        
        
        
        feat_image_batch = torch.zeros(size=(self.batch_size, self.num_jobs, max_opes * num_mas, 4))
        for b in range(self.batch_size):
            for j in range(self.num_jobs):
                bias = self.num_ope_biases_batch[b, j]
                count = self.nums_ope_batch[b, j]
                curr_step_idx = self.ope_step_batch[b, j] - bias
                
                job_data = self.proc_times_batch[b, bias : bias + count, :].reshape(-1)
                feat_image_batch[b, j, :count * num_mas, 0] = job_data.div(self.max_proc)
          
                pos_data = torch.linspace(1, 1/count, count, device=self.device).repeat_interleave(num_mas)
                feat_image_batch[b, j, :count * num_mas, 1] = pos_data

                ect = feat_opes_batch[b, 4, bias : bias + count]
                est = feat_opes_batch[b, 5, bias : bias + count]
                b_values = (ect - est) / (ect + 1e-8)
                feat_image_batch[b, j, :count * num_mas, 2] = b_values.unsqueeze(-1).expand(-1, num_mas).reshape(-1)
                 
                alpha_data = self.ope_ma_adj_batch[b, bias : bias + count, :].reshape(-1)
                feat_image_batch[b, j, :count * num_mas, 3] = alpha_data
        ...
        curr_proc_batch = torch.zeros(size=(self.batch_size, num_jobs, num_mas, 4))
        ...
        counts = torch.count_nonzero(self.ope_ma_adj_batch, dim=1).float()
        feat_mas_batch[:, 0, :] = counts.div(self.nums_opes.unsqueeze(-1))
        feat_mas_batch[:, 1, :] = torch.sum(self.proc_times_batch, dim=1).div(counts + 1e-8).div(self.max_proc)
        feat_mas_batch[:, 2, :] = 1
        ...
        self.feat_image_batch = feat_image_batch
        self.curr_proc_batch = curr_proc_batch
        self.re_proc_times_batch = copy.deepcopy(self.proc_times_batch)
        ...
        self.state = EnvState(batch_idxes=self.batch_idxes,
                              ...
                              feat_image_batch=self.feat_image_batch, curr_proc_batch=self.curr_proc_batch,
                              proc_times_batch=self.proc_times_batch, re_proc_times_batch = self.re_proc_times_batch,
        ...
        self.old_re_proc_times_batch = copy.deepcopy(self.re_proc_times_batch)
        ...
        self.old_feat_image_batch = copy.deepcopy(self.feat_image_batch)
    ...
    def step(self, actions):
        ...
        self.re_proc_times_batch[self.batch_idxes, opes, :] = 0
        ...
        next_opes = torch.where(opes == self.end_ope_biases_batch[self.batch_idxes, jobs], opes, opes + 1)
        bias = self.num_ope_biases_batch[self.batch_idxes, jobs]
        count = self.nums_ope_batch[self.batch_idxes, jobs]
        n_1 = opes - bias 
        n_2 = next_opes - bias

        O_max = self.feat_image_batch.shape[2] // self.num_mas
        local_grid = torch.arange(O_max, device=self.device).view(1, O_max)
        active_global_idx = torch.clamp(bias.unsqueeze(-1) + local_grid, max=self.num_opes - 1)
        batch_offsets = (self.batch_idxes * self.num_opes).unsqueeze(-1)
        mask = local_grid < count.unsqueeze(-1)
        progress_mask = (local_grid >= n_2.unsqueeze(-1)).float()
        b_values = (self.feat_opes_batch[:, 4, :] - self.feat_opes_batch[:, 5, :]).div(self.feat_opes_batch[:, 4, :] + 1e-8)        
        b_expanded = b_values.view(-1)[batch_offsets + active_global_idx].unsqueeze(-1).expand(-1, -1, self.num_mas)
        self.feat_image_batch[self.batch_idxes, jobs, :, 2] = (b_expanded * progress_mask.unsqueeze(-1) * mask.unsqueeze(-1)).reshape(len(self.batch_idxes), -1)

        m_offsets = torch.arange(self.num_mas, device=self.device)
        target_1 = (n_1.unsqueeze(-1) * self.num_mas + m_offsets)
        self.feat_image_batch[self.batch_idxes.unsqueeze(-1), jobs.unsqueeze(-1), target_1, :] = 0
        ...
        active_mask = (~self.mask_job_finish_batch).float().unsqueeze(-1).unsqueeze(-1)
        self.curr_proc_batch[self.batch_idxes, jobs, :, 0] = self.ope_ma_adj_batch[self.batch_idxes, next_opes, :].float()
        self.curr_proc_batch[self.batch_idxes, jobs, :, 1] = self.proc_times_batch[self.batch_idxes, next_opes, :] / self.max_proc
        row_max = torch.max(self.curr_proc_batch[..., 1], dim=-1, keepdim=True)[0] + 1e-8
        self.curr_proc_batch[..., 2] = self.curr_proc_batch[..., 1].div(row_max)
        col_max = torch.max(self.curr_proc_batch[..., 1], dim=-2, keepdim=True)[0] + 1e-8
        self.curr_proc_batch[..., 3] = self.curr_proc_batch[..., 1].div(col_max)
        self.curr_proc_batch *= active_mask

           
        counts = torch.count_nonzero(self.proc_times_batch[self.batch_idxes, ...], dim=-2).float()
        self.feat_mas_batch[self.batch_idxes, 0, :] = counts.div(self.nums_opes[self.batch_idxes].unsqueeze(-1))
        self.feat_mas_batch[self.batch_idxes, 1, :] = torch.sum(self.proc_times_batch[self.batch_idxes, ...], dim=-2) \
                                                      .div(counts + 1e-8).div(self.max_proc)
        self.feat_mas_batch[self.batch_idxes, 2, :] = 1 - self.mask_ma_procing_batch[self.batch_idxes, :].float()                                   
        ...
        self.state.update(self.batch_idxes, self.feat_opes_batch, self.feat_mas_batch, self.feat_image_batch, self.curr_proc_batch, self.proc_times_batch, self.re_proc_times_batch,
    ...
    def next_time(self, flag_trans_2_next_time):
        ...
        max = torch.max(self.feat_opes_batch[:, 4, :])
        b = torch.where(a > self.time[:, None], a, max + 1.0)
        ...
        jobs_index = torch.nonzero(jobs >= 0)
        ...
        self.feat_mas_batch[:, 2, :] = 1 - self.mask_ma_procing_batch.float()
    ...
    def reset(self):
        ...
        self.re_proc_times_batch = copy.deepcopy(self.old_re_proc_times_batch)
        ...
        self.feat_image_batch = copy.deepcopy(self.old_feat_image_batch)
