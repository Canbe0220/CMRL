...
def main():
    ...
    model_paras["actor_in_dim"] = model_paras["out_size_ma"] + model_paras["out_size_ope"] + 4
    ...
    mod_files = os.listdir('./save_model/')[:]
    ...
    if "DRL" in rules:
        for root, ds, fs in os.walk('./save_model/'):
            for f in fs:
                if f.endswith('.pt'):
                    rules.append(f)
    if len(rules) != 1:
    ...
    for i_rules in range(len(rules)):
        ...
        if rule.endswith('.pt'):
            if device.type == 'cuda':
                model_CKPT = torch.load('./save_model/' + mod_files[i_rules])
            else:
                model_CKPT = torch.load('./save_model/' + mod_files[i_rules], map_location='cpu')
